...
class simulator:
    ...
    def run(self):
        ...
        log = {
            ...
            "body_names": self.mujoco_body_names_wo_hand,
            "dof_positions": [],
            "dof_velocities": [],
            "dof_torque": [],
            "body_positions": [],
        ...
        while self.viewer.is_running():
            ...
            log["body_positions"].append(
                np.copy(self.d.xpos[self.mujoco_body_names_wo_hand_indices, :])
            )
            log["body_rotations"].append(
                np.copy(self.d.xquat[self.mujoco_body_names_wo_hand_indices, :])
            )
            log["body_linear_velocities"].append(
                np.copy(self.d.cvel[self.mujoco_body_names_wo_hand_indices, 0:3])
            )
            log["body_angular_velocities"].append(
                np.copy(self.d.cvel[self.mujoco_body_names_wo_hand_indices, 3:6])
            )
    ...
    def update_obs(self, time_step):
        ...
        #################
        self.single_obs[:27] = np.copy(self.motion.joint_pos[time_step])
        #################
        # motion joint vel command 27
        #################
        self.single_obs[27:54] = np.copy(self.motion.joint_vel[time_step])

        body_name = (
            cfg.motion_reference_body
        )  # "torso_link"  # robot_ref_body_index=3 motion_ref_body_index=7
        ...
        # motion_ref_ori_b 6
        #################
        self.single_obs[54:60] = np.copy(motion_ref_ori_b)
        #################
        # base_lin_vel 3
        #################
    ...
    def _init_robot_conf(self):
        ...
        self.mujoco_body_names_wo_hand = [
            s
            for s in mujoco_all_body_names
            if not (s.startswith("R_") or s.startswith("L_"))
        ]
        self.mujoco_body_names_wo_hand_indices = [
            mujoco.mj_name2id(self.m, mujoco.mjtObj.mjOBJ_BODY, name)
            for name in self.mujoco_body_names_wo_hand
        ]
