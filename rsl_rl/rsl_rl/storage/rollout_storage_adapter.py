...
class RolloutStorageAdapter:
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                yield obs_batch, next_obs_batch, privileged_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    None,
                    None,
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                privileged_obs_batch = padded_privileged_obs_trajectories[
                    :, first_traj:last_traj
                ]

                if padded_rnd_state_trajectories is not None:
                    rnd_state_batch = padded_rnd_state_trajectories[
                        :, first_traj:last_traj
                    ]
                else:
                    rnd_state_batch = None
    ...
    def adapter_mini_batch_generator(
        self, num_mini_batches, num_epochs=8, future_steps=20
    ):
        if self.training_type != "rl":
            raise ValueError(
                "This function is only available for reinforcement learning training."
        ...
        if T <= future_steps:
            raise ValueError(
                f"num_transitions_per_env ({T}) must be greater than future_steps ({future_steps})."
            )

        # 计算每个轨迹的可预测起始点数（确保完整 future_steps）
        num_predictions_per_traj = T - future_steps  # 对于 T=24, future_steps=20, 为 4

        # 总样本数（每个环境 num_predictions_per_traj 个样本）
        total_samples = self.num_envs * num_predictions_per_traj  # 4096 * 4

        # 小批量大小
        mini_batch_size = total_samples // num_mini_batches  # 4096 * 4 // 4 = 4096

        # 为所有可能样本生成索引（env_idx * num_predictions_per_traj + pred_idx）
        indices = torch.randperm(total_samples, requires_grad=False, device=self.device)

        # 展平核心数据（保持原 RL 兼容）
        observations = self.observations  # 形状: (T, num_envs, *obs_shape)
        if self.privileged_observations is not None:
            privileged_observations = self.privileged_observations
        else:
            privileged_observations = observations

        actions = self.actions  # (T, num_envs, *actions_shape)
        values = self.values  # (T, num_envs, 1)
        returns = self.returns  # (T, num_envs, 1)
        old_actions_log_prob = self.actions_log_prob  # (T, num_envs, 1)
        advantages = self.advantages  # (T, num_envs, 1)
        old_mu = self.mu  # (T, num_envs, *actions_shape)
        old_sigma = self.sigma  # (T, num_envs, *actions_shape)

        if self.rnd_state_shape is not None:
            rnd_state = self.rnd_state  # (T, num_envs, *rnd_state_shape)
        else:
            rnd_state = None
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                # 计算每个样本的 (env_idx, t)：t 是起始时间步
                env_indices = batch_idx // num_predictions_per_traj
                t_indices = batch_idx % num_predictions_per_traj

                # 提取输入批次：observations[t, env]
                obs_batch = observations[t_indices, env_indices]

                # 提取目标批次：未来 20 步 observations[t+1 : t+1+20, env]
                # 使用 gather 或循环提取（为效率，使用高级索引）
                # 首先，生成范围张量
                future_indices = t_indices.unsqueeze(1) + torch.arange(
                    1, future_steps + 1, device=self.device
                )
                target_batch = observations[
                    future_indices, env_indices.unsqueeze(1).expand(-1, future_steps)
                ]
                # target_batch 形状: (mini_batch_size, future_steps, *obs_shape)
                future_action_batch = actions[
                    future_indices, env_indices.unsqueeze(1).expand(-1, future_steps)
                ]

                # 提取特权观测输入（类似）
                privileged_obs_batch = privileged_observations[t_indices, env_indices]

                # 提取其他 RL 数据（若需；基于 t）
                actions_batch = actions[t_indices, env_indices]
                target_values_batch = values[t_indices, env_indices]
                returns_batch = returns[t_indices, env_indices]
                old_actions_log_prob_batch = old_actions_log_prob[
                    t_indices, env_indices
                ]
                advantages_batch = advantages[t_indices, env_indices]
                old_mu_batch = old_mu[t_indices, env_indices]
                old_sigma_batch = old_sigma[t_indices, env_indices]

                # RND 状态（基于 t）
                if rnd_state is not None:
                    rnd_state_batch = rnd_state[t_indices, env_indices]
                else:
                    rnd_state_batch = None

                # yield 批次数据（添加 target_batch 以支持预测）
                yield obs_batch, privileged_obs_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    None,
                    None,
                ), None, rnd_state_batch, target_batch, future_action_batch
