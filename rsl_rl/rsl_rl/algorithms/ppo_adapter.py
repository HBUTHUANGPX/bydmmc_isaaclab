...
from rsl_rl.modules import ActorCritic, ActorCriticAdapter
from rsl_rl.modules.rnd import RandomNetworkDistillation

# from rsl_rl.storage import RolloutStorage
from rsl_rl.storage import RolloutStorageAdapter as RolloutStorage
...
class PPO_Adapter:
    ...
    policy: ActorCritic | ActorCriticAdapter
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if self.policy.adapter_flag:
                encoder_loss, beta_VAE, wm_loss = self.policy.history_world_model.update(
                    obs_batch, next_obs_batch, beta=1e-2
                )
                autoenc_loss = (encoder_loss + beta_VAE + wm_loss) / self.num_mini_batches
            else:
                encoder_loss, beta_VAE, wm_loss = 0,0,0
                autoenc_loss = 0
            # Surrogate loss
            ratio = torch.exp(
            ...
            mean_ec_loss += encoder_loss
            mean_vae_loss += beta_VAE
            # -- RND loss
        ...
        loss_dict = {
            ...
            "world model": mean_wm_loss,
            "encoder": mean_ec_loss,
            "beta VAE": mean_vae_loss,
        }
        if self.rnd:
